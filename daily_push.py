...
# RSS 聚合用：每次 push 成功后追加一条到 feed_items.json；gen_feed.py 读它生成 docs/feed.xml
FEED_ITEMS_FILE = os.environ.get("FEED_ITEMS_FILE", "feed_items.json")
FEED_KEEP = 100            # feed.xml 里最多保留多少条（按时间倒序）

# 增量推送历史：每次 cron 跑完记录"这次推了哪些 URL"；gen_feed.py 用它生成
# digest.xml 的多 entry 模式（每个 entry = 一次班次新增的）
PUSH_HISTORY_FILE = os.environ.get("PUSH_HISTORY_FILE", "push_history.json")
PUSH_HISTORY_KEEP = 14     # 保留最近 N 次推送记录（约 4-5 天 × 3 班次）
...
def save_feed_items(path, data):
    """保留最近 FEED_KEEP 条，按 pubdate_iso 倒序持久化。"""
    items = data.get("items", [])
    # 按发布时间倒序，截断
    items.sort(key=lambda it: it.get("pubdate_iso", ""), reverse=True)
    data["items"] = items[:FEED_KEEP]
    data["updated_at"] = datetime.now(BEIJING).isoformat()
    with open(path, "w", encoding="utf-8") as f:
        json.dump(data, f, ensure_ascii=False, indent=2)


def load_push_history(path):
    if not os.path.exists(path):
        return {"pushes": []}
    try:
        with open(path, "r", encoding="utf-8") as f:
...
def save_push_history(path, data):
    """按 push_time_iso 倒序保留最近 PUSH_HISTORY_KEEP 次班次。"""
    pushes = data.get("pushes", [])
    pushes.sort(key=lambda p: p.get("push_time_iso", ""), reverse=True)
    data["pushes"] = pushes[:PUSH_HISTORY_KEEP]
    with open(path, "w", encoding="utf-8") as f:
        json.dump(data, f, ensure_ascii=False, indent=2)
...
def push_one(access_token, section_id, dt, url, list_title, log,
    ...
    # 返回字典：原本只返回 title 用于 state 去重；现在还附带 RSS 用的数据
    return {
...
def main():
    ...
    pushed_urls = set(state.get("pushed_urls", []))
    pushed_titles = set(state.get("pushed_titles", []))
    feed_data = load_feed_items(FEED_ITEMS_FILE)
    history_data = load_push_history(PUSH_HISTORY_FILE)
    urls_pushed_this_run = []  # 仅本次班次新推的 URL，写入 push_history.json
    # 装上单篇超时信号处理（仅 Unix）
    ...
    for i, (dt, url, title, content, source, channel) in enumerate(items, 1):
        ...
        try:
            ...
            # 增量历史：记录本班次推出去的 URL
            urls_pushed_this_run.append(url)
            n_ok += 1
    ...
    state["pushed_titles"] = sorted(pushed_titles)
    save_state(state_path, state)
    save_feed_items(FEED_ITEMS_FILE, feed_data)
    # 增量历史：本班次只要有任何成功推送，就记一条
    if urls_pushed_this_run:
        _event = os.environ.get("GITHUB_EVENT_NAME", "")
        trigger_label = {
            "schedule": "定时",
            "workflow_dispatch": "手动",
        }.get(_event, "本地")
        history_data["pushes"].append({
            "push_time_iso": datetime.now(BEIJING).isoformat(),
            "trigger": trigger_label,
            "items_count": len(urls_pushed_this_run),
            "urls": urls_pushed_this_run,
        })
        save_push_history(PUSH_HISTORY_FILE, history_data)
        log(f"  push_history 记入本班次 {len(urls_pushed_this_run)} 条新推 URL")
    log(f"结束。成功 {n_ok}，失败 {n_fail}，feed_items 共 {len(feed_data['items'])} 条")
